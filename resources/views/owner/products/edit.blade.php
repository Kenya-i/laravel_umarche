      ...
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
          ...
          <form method="post" action="{{ route('owner.products.update', ['product' => $product->id ]) }}">
            ...
            <div class="-m-2">
              ...
              <div class="p-2 w-1/2 mx-auto">
                <div class="relative flex justify-around">
                  <div><input class="mr-2" type="radio" name="type" value="{{ \Constant::PRODUCT_LIST['add'] }}" checked>追加</div>
                  <div><input class="mr-2" type="radio" name="type" value="{{ \Constant::PRODUCT_LIST['reduce'] }}">削減</div>
                </div>
              </div>
              <div class="p-2 w-1/2 mx-auto">
                <div class="relative">
            ...
            <div class="p-2 w-1/2 mx-auto">
              <div class="relative flex justify-around">
                <div><input class="mr-2" type="radio" name="is_selling" value="1" @if($product->is_selling === 1){ checked } @endif>販売中</div>
                <div><input class="mr-2" type="radio" name="is_selling" value="0" @if($product->is_selling === 0){ checked } @endif>停止中</div>
              </div>
            </div>
          ...
          <form id="delete_{{ $product->id }}" method="post" action="{{ route('owner.products.destroy', ['product' => $product->id]) }}">
            ...
            <div class="p-2 w-full flex justify-around mt-32">
              <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" class="text-white bg-red-400 border-0 py-2 px-4 focus:outline-none hover:bg-red-500 rounded text-lg">削除する</a>
            </div>
          </form>
        </div>
      </div>
